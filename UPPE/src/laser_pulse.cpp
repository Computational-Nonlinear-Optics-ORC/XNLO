...
void laser_pulse::propagate(double dz_, capillary_fibre& capillary_, keldysh_gas& gas_) {
    ...
    // Call ODE solver
    // (custom, adaptive)
    static double h = dz_ / 5;    // Initial guess
    static double tol = 1e-4;     // Relative error tolerance
...
//std::cout << "   Max A_w_active: " << A_w_active.real().maxCoeff() << ", z: " << z << ", z/dz_: " << z/dz_ << std::endl;
        } else {
            // Tolerence not met
            h *= 0.5;
            last_step = false;
    ...
    static ArrayXXcd temp_2 = ArrayXXcd::Zero(tw.n_active, rkr.n_r);
    temp_2.block(0, 0, tw.n_active, rkr.n_m) = A_w_active;
    for (int ii = 0; ii < tw.n_active; ii++)
        temp_2.row(ii) = ht.backward(temp_2.row(ii));
    static ArrayXXcd temp_3 = ArrayXXcd::Zero(tw.n_t, rkr.n_r);
    ...
    static ArrayXd temp_4 = ArrayXd::Zero(tw.n_t);
    for (int ii = 0; ii < rkr.n_r; ii++)
    ...
    std::cout << "z_position: " << z_position << ", z: " << z << ", atom density: " << gas_.atom_density(z_position) << std::endl;
    //std::cout << "   Max temp_3: " << temp_3.col(ii).real().maxCoeff() << std::endl;
    //std::cout << "   Max temp_4: " << temp_4.real().maxCoeff() << std::endl;
    std::cout << "   Max electron_density: " << electron_density.maxCoeff() << std::endl;
    //std::cout << "   Max gas_.ionisation_rate(): " << gas_.ionization_rate(temp_3.col(ii).real()).maxCoeff() << std::endl;

    //std::cout << "   Max temp_3: " << temp_3.real().maxCoeff() << std::endl;
    //std::cout << "   Max ionisation_rate_w_active_r: " << ionisation_rate_w_active_r.maxCoeff() << std::endl;
...
void laser_pulse::RK_F_45(double h_, capillary_fibre& capillary_, keldysh_gas& gas_) {
    ...
    static ArrayXXcd k_1 = h_ * RHS_UPPE(0, A_w_active, capillary_, gas_);
    static ArrayXXcd k_2 = h_ * RHS_UPPE(0 + (h_/4), A_w_active + (k_1/4), capillary_, gas_);
    static ArrayXXcd k_3 = h_ * RHS_UPPE(0 + (3*h_/8), A_w_active + (3*k_1/32) + (9*k_2/32), capillary_, gas_);
    static ArrayXXcd k_4 = h_ * RHS_UPPE(0 + (12*h_/13), A_w_active + (1932*k_1/2197) - (7200*k_2/2197) + (7296*k_3/2197), capillary_, gas_);
    static ArrayXXcd k_5 = h_ * RHS_UPPE(0 + h_, A_w_active + (439*k_1/216) - (8*k_2) + (3680*k_3/513) - (845*k_4/4104), capillary_, gas_);
    static ArrayXXcd k_6 = h_ * RHS_UPPE(0 + (h_/2), A_w_active - (8*k_1/27) + (2*k_2) - (3544*k_3/2565) + (1859*k_4/4104) - (11*k_5/40), capillary_, gas_);
...
ArrayXXcd laser_pulse::RHS_UPPE(double dz_, ArrayXXcd temp_1, capillary_fibre& capillary_,
    ...
    static ArrayXXcd P_w_active_r = ArrayXXcd::Zero(tw.n_active, rkr.n_r);//tw.n_active, rkr.n_r);
    static ArrayXXcd J_w_active_r = ArrayXXcd::Zero(tw.n_active, rkr.n_r);

//std::cout << "RHS: tw.w_active_min_index: " << tw.w_active_min_index << ", tw.n_active: " << tw.n_active << std::endl;
    for (int ii = 0; ii < rkr.n_r; ii++) {
...
//std::cout << "Max P_w_active_r: " << P_w_active_r.real().maxCoeff() << std::endl;
//std::cout << "Max tw.w_active: " << tw.w_active.maxCoeff() << std::endl;

    for (int ii = 0; ii < tw.n_active; ii++) {
        P_w_active_r.row(ii) = ht.forward(P_w_active_r.row(ii));
        J_w_active_r.row(ii) = ht.forward(J_w_active_r.row(ii));
    }
    static ArrayXXcd P_w_active = P_w_active_r.block(0, 0, tw.n_active, rkr.n_m);
    static ArrayXXcd J_w_active = J_w_active_r.block(0, 0, tw.n_active, rkr.n_m);
    ...
    // Return RHS
    static ArrayXXcd output = std::sqrt(physics.mu_0 / physics.eps_0) * ((P_w_active.colwise() *
...
//std::cout << "Max output: " << output.real().maxCoeff() << std::endl;
